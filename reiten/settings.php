...
if(isset($_GET['save'])) {
	...
	if($save == 'personal_data') {
		...
		if($vorname == "" || $nachname == "") {
			...
			$statement = $pdo->prepare("UPDATE users_reiten SET vorname = :vorname, nachname = :nachname, updated_at=NOW() WHERE id = :userid");
		...
		if($passwortNeu != $passwortNeu2) {
			...
			$statement = $pdo->prepare("UPDATE users_reiten SET passwort = :passwort WHERE id = :userid");
